...
<header class={`z-50 fixed top-0 right-0 left-0 flex`}>
  ...
  >
    ...
    <button onclick={toggleMenu} aria-label="Open menu" class="db md:hidden">
      ...
      <div class="flex-1 flex-row flex justify-center items-center text-base">
        <div class="flex-row flex">
          {#each links as [link, text]}
            <a class="md:mr-4" href={link} onclick={toggleMenu}>{text}</a>
          {/each}
          <button class="mt-10 block md:hidden" onclick={toggleMenu}>close</button>
        </div>
      </div>
    </nav>
  </div>
  ...
  >
    <div class="flex-1 flex-col flex justify-center items-center text-2xl">
      <div class="flex-col flex fancy">
        ...
        {#each links as [link, text]}
          <a class="mb-6" href={link} onclick={toggleMenu}>{text}</a>
        {/each}
        <button class="mt-10 block md:hidden" onclick={toggleMenu}>close</button>
      </div>
    </div>
